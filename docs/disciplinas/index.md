# Disciplinas

Materiais didáticos das disciplinas ministradas pelo Prof. Rodrigo Nogueira
Cardoso no **Curso Técnico em Eletrotécnica Integrado** do IFTM Campus
Ituiutaba.

## Disciplinas ativas

<div class="grid cards" markdown>

-   :material-cog-transfer:{ .lg .middle } **Automação Industrial I**

    ---

    Pneumática, eletropneumática e métodos de sequenciamento
    (Cascata e Passo a Passo).

    [:octicons-arrow-right-24: Acessar materiais](automacao-industrial I/index.md)

-   :material-folder-multiple:{ .lg .middle } **Introdução a lógica e programação**

    ---

    Sistemas Numéricos, binário, octal, hexadecimal, aritmética binária, funções lógicas, fatoração, mapa de karnaugh, introdução a programação, variáveis, constantes, estruturas de controle,      Arduino, portas digitais e analógicas, comunicação serial, PWM

    [:octicons-arrow-right-24: Ver mais](outras/index.md)

</div>

## Como utilizar este espaço

Cada disciplina possui sua própria área organizada com:

- **Plano de ensino** — ementa, objetivos, cronograma e avaliação
- **Apostila** — material teórico de referência completo
- **Aulas** — slides, vídeos e materiais por encontro
- **Atividades** — listas de exercícios e práticas
- **Avaliações** — provas, trabalhos e critérios
- **Referências** — bibliografia e links complementares

!!! tip "Dúvidas?"

    Use a busca no topo da página para localizar conteúdos específicos
    ou entre em contato pela área [Sobre](../sobre.md).
